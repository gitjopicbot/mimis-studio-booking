...
const SITE_URL = process.env.SITE_URL || 'https://mimisstudio1.com';
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Self-service cancellation link for the confirmation email. cancel_token is
    // a random UUID generated by the database (see the cancel-token migration).
    const cancelUrl = appointment.cancel_token
      ? `${SITE_URL}/cancel?token=${appointment.cancel_token}`
      : null;
    ...
    try {
      const clientEmailResult = await resend.emails.send({
        ...
        html: clientEmailHtml({
          ...
          serviceNames, totalDuration, cancelUrl
        })
      });
...
async function findExistingClient(firstName, lastName, email, phone) {
  ...
  // Build lookup maps
  // NOTE: clients created by Mimi in the admin may have only an email OR only
  // a phone (never neither). Every field read here must tolerate null.
...
function clientEmailHtml({ firstName, dateFormatted, timeFormatted, endTimeFormatted, serviceNames, totalDuration, cancelUrl }) {
  // Self-service cancellation. The 24-hour policy is enforced by the
  // cancel-appointment function, not here — inside 24 hours the link asks the
  // client to call the studio instead.
  const cancelSection = cancelUrl ? `
      <div style="text-align: center; margin: 24px 0 8px;">
        <a href="${cancelUrl}" style="display:inline-block; background:#ECE3D0; color:#5C6B82; text-decoration:none; padding:12px 26px; border-radius:8px; font-size:14px; border:1px solid #C9A961;">
          Need to cancel? Cancel this appointment
        </a>
      </div>
      <p style="color:#95A0B0; font-size:12px; text-align:center; margin:0 0 8px;">
        Cancellations are accepted online up to 24 hours before your appointment.
      </p>` : '';
  ...
  <div style="font-family: 'Georgia', serif; max-width: 600px; margin: 0 auto; background: #FAF6EC;">
    ...
    <div style="padding: 30px;">
      ...
      <p style="color: #5C6B82; font-size: 14px; line-height: 1.6;">
        Need to change your appointment, or cancel within 24 hours? Please call or text Mimi at
        <a href="tel:+17072924914" style="color: #466B8E; font-weight: bold; text-decoration: none;">(707) 292-4914</a>.
      </p>
